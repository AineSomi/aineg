...
def banner():
	...
	print("""\n 
    ____           _  ┌════════════════════════┐
   / __/__  __ _  (_) │ * Coded By Somi awan * │
  _\ \/ _ \/  ' \/ /  │ * GitHub - SOMI AWAN * │                 /___/\___/_/_/_/_/   │════════════════════════│
 ═════════════════════│    * \033[1;92mLite v 3.6.0 \033[1;97m*    │
 ═════════════════════└════════════════════════┘       """)           
...
def _site_view_():
			banner()
			print (' [*]══════════════════════════════════════════')
			print ("\033[1m [!] Tool    : Brand Boy")
			print(" [!] Expired : ")
			print (" [!] Status  : \033[1;92mPremium\033[1;97m")
			print (' [*]══════════════════════════════════════════');
			print(" [1] Start File Cloning \n [2] Start Public Cloning")
			print(" [3] Create ids File \n [4] Logout Token")
			key = input("\n [*] input : ")
			if key in [""]:
...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	banner();print ('\n [01] Try ~> Extension 1 ');print (' [02] Try ~> Extension 2%s <best>\033[1;97m'%(J))
	hc = input ("\n [#] method : ")
	if hc in ['1','01']:
...
def first():
	banner();print( ' [!] \033[1;96mTurn Airplane Mode On/Off Every 5 Minutes\033[1;0m\n')
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[Hard] %s/%s / OK:%s / CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
